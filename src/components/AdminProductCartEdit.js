...
const AdminProductCartEdit = ({ onClose, productData ,fetchfux}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        // Ensure `data` is defined and valid before sending the request
        if (!data) {
        ...
        try {
            ...
            const response= await fetch(summeryApi.updateproductdetail.url,{
                method:"POST",
                headers:{
    ...
    return (
        <div>
            <div className=' fixed bottom-0 top-0 left-0 right-0  overscroll-none  z-10  '>
                <div className='  flex justify-center items-center h-full w-full '>
                    <div className=' max-w-full bg-slate-300  w-[calc(600px)] p-6  rounded-md h-[calc(550px)] overflow-y-scroll '>
                        <div className=' flex justify-between '>
                            <div className=' h-fit p-2 font-bold'>Update Products:</div>
                            <div
                                className='hover:cursor-pointer hover:text-red-500 h-fit '
                                onClick={onClose}
                            >
                                <IoIosCloseCircleOutline size={40} />
                            </div>
                        </div>
                        <form className='grid p-2 my-1 gap-1 ' onSubmit={handleSubmit} onChange={handleOnChange}>
                            ...
                            <div
                                className=' w-full bg-slate-200 h-[clac(1000px)] rounded-md relative cursor-pointer'
                                onClick={handleClick}
                            >
                                <div className=' flex justify-center '>
                                    <FaCloudUploadAlt size={60} />
                                </div>
                                <div className='text-center  '>upload Product Image</div>
                                <input
                                    type='file'
                                    name='productImage'
                            ...
                            <div className='flex flex-wrap gap-2'>
                                {data?.productImage && data.productImage.length > 0 ? (
                                    data.productImage.map((el, index) => (
                                        <div
                                            key={index}
                                            className='relative group my-auto w-8 bg-red-500'
                                        >
                                            <img
                                                src={el}
                                                alt={`Product ${index}`}
                                            ...
                                            />
                                            <div
                                                className='absolute  bottom-0 right-0 bg-red-500 text-white rounded-full hidden group-hover:block hover:cursor-pointer p-1'
                                                key={index}
                            ...
                            <div className='gap-1'>
                                ...
                                <div className='w-full  bg-blue-300'>
                                    <input
                                        ...
                                        value={data.price}
                                        onChange={handleOnChange}
                                        className='w-full rounded-md py-1 border-2 border-sky-500'
                                    ></input>
                                </div>
                            </div>
                            <div className='gap-1'>
                            ...
                            <div className='grid gap-1'>
                                ...
                                <textarea
                                    ...
                                    onChange={handleOnChange}
                                    className='w-full rounded-md py-1 border-2 border-sky-500 resize-none p-2'
                                ></textarea>
                            </div>

                            <button className='rounded-md w-full bg-red-500 text-white hover:cursor-pointer p-3 hover:bg-red-600 '>
                                Update Product
                            </button>
                        </form>
                    </div>
...
export default AdminProductCartEdit;
