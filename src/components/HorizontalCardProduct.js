...
const HorizontalCardProduct = ({ catogry, heading }) => {
    ...
    // const [numberOfProduct,setNOP]=useState()
            ...
            <div className='flex flex-row  h-full flex-wrap rounded-sm justify-center md:justify-between md:gap-6  scrollbar-none transition-all  '>
                {
                    ...
                    loading ? (
                        ...
                        data.map((product, index) => {
                            return (
                                <Link to={"/product/" + product?._id} className='w-full   min-w-[280px]  md:min-w-[320px] max-w-[280px] md:max-w-[320px]  rounded-sm shadow ' key={index} >
